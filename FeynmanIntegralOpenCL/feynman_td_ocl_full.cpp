...
#define CLFILE_TD_FULL "integral_td_full.cl"
#define KERNEL_TD_FULL "feynman_td_full"

#define MAX_SOURCE_SIZE 0x100000
#define MAX_PLATFORM 2		// number of platform
#define MAX_DEVICE 2		// number of device

void feynman_ocl_td_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA);
...
void feynman_ocl_td_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA)
{
    ...
    td_real s, tt, fme, fmf, ramda, sum, result;
    ...
    string filename = CLFILE_TD_FULL;
    ...
    _h = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    ...
    _w1 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 3, NULL, &ret);
  ...
  /***************************************/
    ...
    ret = clEnqueueWriteBuffer(command_queue, _h, CL_TRUE, 0, sizeof(double) * 3, &h, 0, NULL, NULL);
    ...
    kernel = clCreateKernel(program, KERNEL_TD_FULL, &ret);
    ...
    // read output buffer
    ret = clEnqueueReadBuffer(command_queue, _w1, CL_TRUE, 0, n * sizeof(double) * 3, w1, 1, &eve1, NULL);
