...
#define CLFILE_DD "integral_dd.cl"
#define KERNEL_DD "feynman_dd_d2"

#define MAX_SOURCE_SIZE 0x100000
#define MAX_PLATFORM 2		// number of platform
#define MAX_DEVICE 2		// number of device

void feynman_ocl_dd(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA);
...
void feynman_ocl_dd(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA)
{
    ...
    dd_real *w2 = new dd_real[n];
    dd_real *w3 = new dd_real[n];
    dd_real y, pi, t;
    double knl_s, knl_e;
    double ocl_time = 0.0;

    dd_real x, s, tt, fme, fmf, ramda;
    ...
    _cnt0 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    _cnt2 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    _cnt4 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    _cnt5 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    _xx = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    _yy = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 2, NULL, &ret);
    ...
    _w3 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 2, NULL, &ret);
  ...
  /***************************************/
    ...
    kernel = clCreateKernel(program, KERNEL_DD, &ret);
	...
	for (i2 = 0; i2 < n; i2++) {
	    ...
	    ret = clEnqueueWriteBuffer(command_queue, _cnt0, CL_TRUE, 0, sizeof(double) * 2, &cnt0, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt2, CL_TRUE, 0, sizeof(double) * 2, &cnt2, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt4, CL_TRUE, 0, sizeof(double) * 2, &cnt4, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt5, CL_TRUE, 0, sizeof(double) * 2, &cnt5, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _xx, CL_TRUE, 0, sizeof(double) * 2, &xx, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _yy, CL_TRUE, 0, sizeof(double) * 2, &yy, 0, NULL, NULL);
	    ...
	    // read output buffer
	    ret = clEnqueueReadBuffer(command_queue, _w3, CL_TRUE, 0, n * sizeof(double) * 2, w3, 1, &eve1, NULL);
