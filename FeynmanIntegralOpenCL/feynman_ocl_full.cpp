...
#define CLFILE_FULL "integral_full.cl"
#define KERNEL_FULL "feynman_d_full"

#define MAX_SOURCE_SIZE 0x100000
#define MAX_PLATFORM 2		// number of platform
#define MAX_DEVICE 2		// number of device

void feynman_ocl_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA);
...
void feynman_ocl_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA)
{
    ...
    double s, tt, fme, fmf, ramda, sum, result;
    ...
    _w1 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double), NULL, &ret);
  ...
  /***************************************/
    ...
    kernel = clCreateKernel(program, KERNEL_FULL, &ret);
    ...
    ret = clSetKernelArg(kernel, 1, sizeof(double), (void *) &h);
    ret = clSetKernelArg(kernel, 2, sizeof(double), (void *) &s);
    ret = clSetKernelArg(kernel, 3, sizeof(double), (void *) &tt);
    ret = clSetKernelArg(kernel, 4, sizeof(double), (void *) &ramda);
    ret = clSetKernelArg(kernel, 5, sizeof(double), (void *) &fme);
    ret = clSetKernelArg(kernel, 6, sizeof(double), (void *) &fmf);
    ...
    // read output buffer
    ret = clEnqueueReadBuffer(command_queue, _w1, CL_TRUE, 0, n * sizeof(double), w1, 1, &eve1, NULL);
