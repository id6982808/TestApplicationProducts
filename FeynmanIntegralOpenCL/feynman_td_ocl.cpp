...
#define CLFILE_TD "integral_td.cl"
#define KERNEL_TD "feynman_td"

#define MAX_SOURCE_SIZE 0x100000
#define MAX_PLATFORM 2		// number of platform
#define MAX_DEVICE 2		// number of device

void feynman_ocl_td(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA);
...
void feynman_ocl_td(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA)
{
    ...
    td_real h = H;
    int i1, i2;
    td_real *gw30 = new td_real[n];
    td_real *x30 = new td_real[n];
    td_real *w1 = new td_real[n];
    td_real *w2 = new td_real[n];
    td_real *w3 = new td_real[n];
    qd_real y, pi, t;
    double knl_s, knl_e;
    double ocl_time = 0.0;

    td_real x, s, tt, fme, fmf, ramda;
    ...
    _cnt0 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _cnt2 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _cnt4 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _cnt5 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _xx = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _yy = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);

    _s = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _tt = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _ramda = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _fme = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);
    _fmf = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 3, NULL, &ret);

    _w3 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 3, NULL, &ret);
    _gw30 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 3, NULL, &ret);
    _x30 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 3, NULL, &ret);
  ...
  /***************************************/
    ...
    ret = clEnqueueWriteBuffer(command_queue, _s, CL_TRUE, 0, sizeof(double) * 3, &s, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, _tt, CL_TRUE, 0, sizeof(double) * 3, &tt, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, _ramda, CL_TRUE, 0, sizeof(double) * 3, &ramda, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, _fme, CL_TRUE, 0, sizeof(double) * 3, &fme, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, _fmf, CL_TRUE, 0, sizeof(double) * 3, &fmf, 0, NULL, NULL);

    ret = clEnqueueWriteBuffer(command_queue, _gw30, CL_TRUE, 0, n * sizeof(double) * 3, gw30, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, _x30, CL_TRUE, 0, n * sizeof(double) * 3, x30, 0, NULL, NULL);


    kernel = clCreateKernel(program, KERNEL_TD, &ret);
	...
	for (i2 = 0; i2 < n; i2++) {
	    ...
	    ret = clEnqueueWriteBuffer(command_queue, _cnt0, CL_TRUE, 0, sizeof(double) * 3, &cnt0, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt2, CL_TRUE, 0, sizeof(double) * 3, &cnt2, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt4, CL_TRUE, 0, sizeof(double) * 3, &cnt4, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _cnt5, CL_TRUE, 0, sizeof(double) * 3, &cnt5, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _xx, CL_TRUE, 0, sizeof(double) * 3, &xx, 0, NULL, NULL);
	    ret = clEnqueueWriteBuffer(command_queue, _yy, CL_TRUE, 0, sizeof(double) * 3, &yy, 0, NULL, NULL);
	    ...
	    // read output buffer
	    ret = clEnqueueReadBuffer(command_queue, _w3, CL_TRUE, 0, n * sizeof(double) * 3, w3, 1, &eve1, NULL);
    ...
    std::cout << "error to analytical:     " << analytical(1) - result.toQDreal() << std::endl;
