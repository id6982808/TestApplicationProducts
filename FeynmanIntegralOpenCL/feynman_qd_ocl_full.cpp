...
#define CLFILE_QD_FULL "integral_qd_full.cl"
#define KERNEL_QD_FULL "feynman_qd_full"

#define MAX_SOURCE_SIZE 0x100000
#define MAX_PLATFORM 2		// number of platform
#define MAX_DEVICE 2		// number of device

void feynman_ocl_qd_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA);
...
void feynman_ocl_qd_full(int N, int GLOBAL, int LOCAL, int PID, int DID, double H, double RAMDA)
{
    ...
    qd_real s, tt, fme, fmf, ramda, sum, result;
    ...
    string filename = CLFILE_QD_FULL;
    ...
    _h = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * 4, NULL, &ret);
    ...
    _w1 = clCreateBuffer(context, CL_MEM_READ_WRITE, n * sizeof(double) * 4, NULL, &ret);
  ...
  /***************************************/
    ...
    ret = clEnqueueWriteBuffer(command_queue, _h, CL_TRUE, 0, sizeof(double) * 4, &h, 0, NULL, NULL);
    ...
    kernel = clCreateKernel(program, KERNEL_QD_FULL, &ret);
    ...
    // read output buffer
    ret = clEnqueueReadBuffer(command_queue, _w1, CL_TRUE, 0, n * sizeof(double) * 4, w1, 1, &eve1, NULL);
